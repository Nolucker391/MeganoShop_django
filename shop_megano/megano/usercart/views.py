...
class UserBasketView(APIView):
    def post(self, request: Request):
        ...
        if not self.request.user.is_authenticated:
            ...
            user_basket, _ = UserCart.objects.get_or_create(
            ...
            if product in (e.product for e in basket_list.filter(product=product)):
                ...
                basket_item = BasketItem(
                    product=product, count=data["count"], basket=user_basket
                )
                basket_item.save()
    ...
    def delete(self, *args, **kwargs):
        ...
        if not self.request.user.is_authenticated:
            ...
            basket_item = BasketItem.objects.get(
                basket=user_cart, product__pk=int(self.request.data.get("id"))
            )
